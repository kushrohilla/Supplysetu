...
export function RetailerInviteGenerator() {
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] min-h-[620px] flex-col gap-4">
      ...
      {!loading && joinedRetailers.length === 0 ? (
        <div className="flex flex-1 items-center justify-center p-8 rounded-lg border border-slate-200 bg-white">
          <EmptyState
            icon="🏪"
            title="No retailers linked yet"
            helper="Retailers join your network by entering a unique invite code in the SupplySetu Retailer App."
            ctaLabel="Generate retailer invite"
            onCtaPress={() => void regenerateInvite()}
            hint={
              <>
                <p className="mb-2 font-medium text-slate-800">Quick steps:</p>
                <ol className="list-inside list-decimal space-y-1">
                  <li>You generate a secure invite code</li>
                  <li>Share via WhatsApp, SMS, or in person</li>
                  <li>Retailer installs app and enters code</li>
                  <li>Retailer appears here instantly</li>
                </ol>
                <p className="mt-4 font-medium text-slate-700">Average activation time: under 2 minutes</p>
              </>
            }
          />
        </div>
