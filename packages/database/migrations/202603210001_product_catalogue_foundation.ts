...
export async function up(knex: Knex): Promise<void> {
  const hasGlobalBrands = await knex.schema.hasTable("global_brands");
  if (!hasGlobalBrands) {
  ...
  const hasGlobalProducts = await knex.schema.hasTable("global_products");
  ...
  const hasTenantProductSchemes = await knex.schema.hasTable("tenant_product_schemes");
  if (!hasTenantProductSchemes) {
  ...
  const hasStockSnapshots = await knex.schema.hasTable("tenant_product_stock_snapshots");
  ...
  const hasImportJobs = await knex.schema.hasTable("tenant_product_import_jobs");
  if (!hasImportJobs) {
  ...
  const hasImportRows = await knex.schema.hasTable("tenant_product_import_rows");
  if (!hasImportRows) {
